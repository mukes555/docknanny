...
3. The release workflow checks that the tag, `MARKETING_VERSION`, and the
   ...
   also runs on every push to main and every pull request), builds
   `DockNanny-X.Y.Z.dmg`, writes
   ...
   keep entries user facing. A tag with a suffix (`v0.5.0-rc.1`, `-beta`,
   `-alpha`) is marked pre-release and leaves the tap alone; the DMG is still
   named after `MARKETING_VERSION`, so the suffix is a label, not a version.
